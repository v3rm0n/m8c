...
static SDL_Thread *th1 = NULL;

int usb_loop(void *userdata) {
...
static int bulk_transfer(int endpoint, uint8_t *serial_buf, int count) {
  int completed = 0;

  struct libusb_transfer *transfer;
  transfer = libusb_alloc_transfer(0);
  libusb_fill_bulk_transfer(transfer, devh, endpoint, serial_buf, count,
                            xfr_cb_in, &completed, 1);
...
int blocking_write(void *buf,
  ...
  return bulk_transfer(ep_out_addr, buf, count);
}

int serial_read(uint8_t *serial_buf, int count) {
  return bulk_transfer(ep_in_addr, serial_buf, count);
}
...
int init_interface() {
  ...
  th1 = SDL_CreateThread(&usb_loop, "Audio", NULL);

  rc = init_audio(devh);

  if (rc < 0) {
    SDL_Log("Init audio failed %d", rc);
    return 0;
  }
...
int disconnect() {
  ...
  if (rc < 0) {
    SDL_Log("Closing audio failed %d", rc);
  }
